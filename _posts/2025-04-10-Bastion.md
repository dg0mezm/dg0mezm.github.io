...
Vemos que hay bastantes ficheros llamados `confComs.xml`, el cual contiene la información de las sesiones que el usaurio tiene guardadas para conectarse remotamente a otros sistemas, por ello intentaremos buscar la palabra `password` entre todos los ficheros y ver en cuáles aparece para luego intentar leer el fichero y obtener la contraseña. Ejecutamos el siguiente comando después de movernos a ese directorio:
...
Parece que también aparece en bastantes ficheros, aunque revisaremos el fichero `confComs.xml`, ya que es el más actual, por ello, leeremos su contenido y bucaremos la palabra `Administrator` para ver si de alguna forma podemos ver la `password`:
